...
<table class="table table-bordered table-hover m10">
    <tr>
        ...
        <td>
            <select name="parentid">
                ...
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value='<?php echo ($v['id']); ?>' <?php if($v['id'] == $cl['pid']): ?>selected<?php endif; ?> />&nbsp;<?php echo ($v['c_title']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>                           
            </select>
        </td>
    </tr>
    <tr>
        <td class="tableleft">名称</td>
        <td><input type="text" name="c_title" value="<?php echo ($cl['c_title']); ?>"/></td>
    </tr>
    <tr>
        <td class="tableleft">描述</td>
        <td><textarea cols="40" rows="5" name="c_desc" id="c_desc" style="width:380px;" ><?php echo ($cl['c_desc']); ?></textarea></td>
    </tr>
    <tr>
        ...
        <td>
            ...
            <br/>
            <?php if($cl['small_img'] != ''): ?><img src="<?php echo ($cl['small_img']); ?>" style="height:80px;"/><?php endif; ?>
        </td>
    </tr>
    <tr>
        <td class="tableleft">排序</td>
        <td>
            <input class="text" type="text" name="myorder" value="<?php echo ($cl['myorder']); ?>" style="width:90px;" onblur="if(/[^0123456789]/g.test(value))value=value.replace(/[^0123456789]/g,'');" onkeyup="if(/[^0123456789]/g.test(value))value=value.replace(/[^0123456789]/g,'');"/>
        </td>
    </tr>   
    <tr>
        <td class="tableleft">状态</td>
        <td>
            <input type="radio" name="status" value="1" <?php if($cl['status'] == 1): ?>checked<?php endif; ?> /> 启用
            <input type="radio" name="status" value="0" <?php if($cl['status'] == 0): ?>checked<?php endif; ?> /> 禁用
        </td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
            <input type="text" name="meta_title" style="width:380px;" value="<?php echo ($cl['meta_keywords']); ?>"/>
        </td>
    </tr>
    <tr>
        <td class="tableleft">Meta 关键字</td>
        <td>
            <input type="text" name="meta_keywords" style="width:380px;" value="<?php echo ($cl['meta_keywords']); ?>"/>
        </td>
    </tr>
    <tr>
        <td class="tableleft">Meta 描述</td>
        <td>
            <input type="text" name="meta_description" style="width:380px;" value="<?php echo ($cl['meta_description']); ?>"/>
        </td>
    </tr>    
    <tr>
