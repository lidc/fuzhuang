...
class DesignCategoryController extends Controller{
    public function index(){
        ...
        $numPerPage = 10;
        $count = $design_category->where($where)->count();
        $Page = new Page($count,$numPerPage);
        $list = $design_category->field('id,pid,c_title,small_img,myorder,status')->where($where)->order('myorder')->page($nowPage.','.$Page->listRows)->select();
        ...
        foreach ($list as $key=>$value){
            ...
            $cl = $design_category->field('id,pid,c_title,small_img,myorder,status')->where("pid=".$value['id'])->order('myorder')->select();
            if($cl){
    ...
    public function add(){
        ...
        if($_POST){
            ...
            $result = $design_category->data($data)->add();
            if($result){
        ...
        $list = $design_category->field('id,c_title')->where($where)->order('myorder')->select();
    ...
    public function edit(){
    	...
    	if($_POST){
    		...
    		$result = $design_category->data($data)->where('id='.$id)->save();
    		if($result){
    	...
    	$cList = $design_category->where('id='.$id)->find();
    ...
    public function delete(){
    	...
    	$count = $design_category->where('pid='.$id)->count();
    	$where = "cid=".$id." or cpid=".$id."";
    	$count2 = $design->where($where)->count();
    	if($count>0 || $count2>0){
    		echo "<script>alert('删除失败！因子级关系或产品中有关联，请先删除关联关系！');location.href='index';</script>";
    		exit();
    	}else{    		
    		$result = $design_category->where('id='.$id)->delete();
    		if($result){
